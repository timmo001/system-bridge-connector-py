...
class Filesystem(BridgeBase):
    ...
    def blockDevices(self):
        return [BlockDevices(x) for x in self.attributes.get("blockDevices", [])]

    @property
    def diskLayout(self):
        return [DiskLayout(x) for x in self.attributes.get("diskLayout", [])]
    ...
    @property
    def fsSize(self):
        return [FsSize(x) for x in self.attributes.get("fsSize", [])]
