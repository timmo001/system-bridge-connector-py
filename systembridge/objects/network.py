...
class Network(BridgeBase):
    ...
    @property
    def interfaces(self):
        return [Interfaces(x) for x in self.attributes.get("interfaces", [])]

    @property
    def stats(self):
        return [Stats(x) for x in self.attributes.get("stats", [])]
